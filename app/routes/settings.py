...
def update_settings(update: AppSettingsIn, db: Session = Depends(get_db)):
    ...
    # smtp_password is write-only: only touch it if a real value was sent
    incoming_password = data.pop("smtp_password", None)
    if incoming_password:
        settings_store.set_setting(db, "smtp_password", incoming_password)
