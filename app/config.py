...
class Settings:
    ...
    # Kept for reference/backward compatibility — email actually sends via
    # Brevo's HTTPS API now (see BREVO_API_KEY below), not SMTP. Two separate
    # SMTP failures on Render's free tier (an IPv6-routing issue, then a
    # plain connection timeout) made HTTPS the reliable choice.
